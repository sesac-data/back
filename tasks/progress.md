...
## 2026-06-17 Rule Engine OR/AND Condition Groups

- Scope: added condition-group (OR/AND) evaluation to the Rule Engine only. The existing `evaluate_operator_conditions` (and its tests) is unchanged; no calculation formula, combination logic, API, DB schema, or frontend was changed.
- Motivation: several policies state OR requirements (e.g. `만 12세 이하 또는 초등학교 6학년 이하`) that the flat AND-only evaluator cannot express; the golds routed these to unresolved_rules. This is roadmap Stage 8.1.
- Added to `match_agent_v0.8/match_agent_v0.8/services/condition_evaluator.py`: `is_condition_group`, `evaluate_condition_group`, `evaluate_conditions_with_groups`, and `SUPPORTED_GROUP_MODES`. An `or` group passes when any member passes; `and` when all pass; empty groups and unsupported modes do not pass. Member results are kept in `member_results` for explainability. A list with no groups yields the same result as `evaluate_operator_conditions`.
- Group schema: `{condition_group_id, mode: "or"|"and", conditions: [...], evidence_snippets: [...]}`.
- Added `match_agent_v0.8/match_agent_v0.8/test_condition_group_evaluator.py` (OR pass/fail, OR with missing field on one branch, AND group, unsupported mode, empty group, mixed flat+group aggregation, and a no-groups-matches-flat-evaluator regression).
- Updated `docs/RULE_ENGINE.md` with a Condition Groups section.
- Verification: `test_condition_group_evaluator.py` passes; `test_condition_operator_evaluator.py` and the monthly_fixed/period_tiered/conditional_bonus calculation tests still pass.
- Not yet wired into calculation_service (the calculation path still calls `evaluate_operator_conditions`); integrating groups into the calculation precondition path and extraction prompt is a clearly-scoped follow-up.

## 2026-06-17 Condition Type Coverage Audit

- Scope: added a read-only audit that compares policy JSON condition types/operators against what the rule engine actually handles. No policy JSON, evaluation, or recommendation behavior was changed.
- Motivation: `docs/analysis/CURRENT_STATE.md` flagged that policy JSON condition types not handled by the evaluator fail silently. This is the `condition-type-coverage-audit` feature.
- Added `SUPPORTED_CONDITION_TYPES` to `services/condition_evaluator.py` (the canonical list of legacy type-based condition types handled by `evaluate_condition`, to be kept in sync with its branches).
- Added `match_agent_v0.8/match_agent_v0.8/services/condition_coverage_auditor.py` (`collect_condition_usage`, `audit_condition_coverage`) and `scripts/audit_condition_type_coverage.py` (CLI, `--strict` to exit 1 on gaps, writes `output/condition_coverage/condition-coverage-report.json`).
- Added `match_agent_v0.8/match_agent_v0.8/test_condition_coverage_auditor.py`.
- Live audit result over `data/policy_json/` (11 policies): only 7 condition types are supported, while 52 are UNSUPPORTED (e.g. `daily_work_hour_reduction`, `attendance_missing_days_per_month`, `minimum_duration_months`, `leave_duration_days`, `monthly_overtime_hours`, plus a garbled `requires_flexible_work制度`). This quantifies the silent-failure risk and gives a prioritized list for future Rule Engine extensions.
- Verification: `test_condition_coverage_auditor.py` passes; the audit script runs and writes the report.

## 2026-06-17 Wire OR/AND Condition Groups Into the Calculation Pipeline

- Scope: connected the previously-added condition groups to the live calculation path so policies can actually use OR/AND groups. No amount formula, combination logic, API, DB schema, or frontend was changed.
- Updated `match_agent_v0.8/match_agent_v0.8/services/calculation_service.py`: the two precondition evaluation call sites (support-item conditions and conditional-bonus conditions) now call `evaluate_conditions_with_groups` instead of `evaluate_operator_conditions`. Because the result keys (`eligible`, `passed_conditions`, `failed_conditions`) are identical and a no-group list behaves the same, this is backward compatible.
- Updated `collect_condition_evidence` to also gather evidence from `groups[].evidence_snippets` and `groups[].member_results[].evidence_snippets`, so OR/AND group evidence is preserved in the calculation result. With no groups present this adds nothing.
- Added `match_agent_v0.8/match_agent_v0.8/test_condition_group_calculation_integration.py`: an OR group gating a monthly_fixed support — eligible when any branch passes (incl. one branch missing), ineligible when all branches fail, and group evidence collected into the result.
- Verification: 12 calculation/recommendation/condition tests pass (monthly_fixed, period_tiered, conditional_bonus, standardized result, minimum recommendation flow, operator evaluator, condition group evaluator, the new integration test, coverage auditor, combination amount summarizer, employer net cost, optimal selection) with no regression.
- Now OR/AND condition groups are usable end-to-end in recommendation calculation, not just as an isolated evaluator. Surfacing group outcomes in the ineligible result detail and adding groups to the extraction prompt remain optional follow-ups.

## 2026-06-17 OR Condition Groups End-to-End: Validator + Extraction Prompt

- Scope: closed the extraction-to-engine loop for OR conditions so the LLM emits condition groups, the validator accepts them, and the engine evaluates them. No amount formula, combination logic, API, DB schema, or frontend was changed; candidates remain review_status=needs_review.
- Validator: extended `services/policy_extraction_candidate_validator.py` to recognize and validate condition groups (`_is_condition_group`, `_validate_condition_group`, `ALLOWED_CONDITION_GROUP_FIELDS`, `ALLOWED_CONDITION_GROUP_MODES`). A group must have `condition_group_id`, a `mode` of `or`/`and`, and a non-empty `conditions` list whose members are validated as normal conditions sharing the global condition-id uniqueness check. New error types: `missing_condition_group_id`, `unsupported_condition_group_mode`, `empty_condition_group`.
- Validator tests: added valid-group, unsupported-mode, empty-members, and duplicate-member-id cases to `test_policy_extraction_candidate_validator.py`.
- Prompt: updated `prompts/policy_extraction_v6.md` to require OR alternatives (`또는`/`이거나`/`중 하나`) to be one `mode="or"` condition group rather than split into separate AND conditions or routed to unresolved_rules. Added the OR Conditions shape, an AND/OR caveat in the sentence-splitting rule, and a self-check item.
- Prompt-adherence finding: gpt-4.1 initially ignored the OR-group rule (two live runs split `만 12세 이하 또는 초등학교 6학년 이하` into two ANDed conditions). Moving a blunt CRITICAL RULE to the very top of the prompt fixed it: the model then emitted one `CG-001 mode="or"` group with `employee.child_age <= 12` and `employee.child_school_grade <= 6` members, validation passed with 0 errors. Lesson recorded: high-priority structural rules must be at the top of the prompt, not buried.
- Updated `docs/POLICY_SCHEMA.md` with a Condition Groups section.
- Verification: 9 tests pass (validator, condition group evaluator, calculation integration, coverage auditor, monthly_fixed, conditional_bonus, candidate assembler, source preprocessor, html table extractor). Live extraction of childcare_flexible_start_support with the updated prompt produced a valid OR condition group.
- The OR pipeline is now consistent end-to-end: prompt -> model output -> validator -> calculation engine. Full verify.ps1 not run (sandbox blocks -ExecutionPolicy Bypass).

## 2026-06-17 Cost-Share Calculation Type (percentage-of-cost subsidies)

- Scope: added a new `cost_share` calculation type end-to-end (engine, validator, extraction prompt) for subsidies that pay a percentage of the employer's actual cost with a cap, e.g. `flexible_work_system_support` (투자비의 80%, 한도 1,000만원). No existing amount formula was changed; candidates remain review_status=needs_review. This is the first roadmap Stage 8 calculation-gap closure beyond OR conditions.
- Engine: added `COST_SHARE` constant and `calculate_cost_share_policy_support` to `services/calculation_service.py`, and dispatched it from `calculate_base_policy_support`. Formula: `min(actual_cost * support_ratio, max_support_amount)`. `support_ratio` and `max_support_amount` are source-backed policy values; the actual cost is a provided input via `cost_field` (default `company.investment_cost`). No cost is inferred: a missing/invalid cost input or invalid support_ratio returns `calculation_error`. An absent `max_support_amount` means no cap (not inferred). Results normalize through `normalize_policy_calculation_result` like other types.
- Tests: added `match_agent_v0.8/match_agent_v0.8/test_cost_share_calculation.py` (ratio below cap, cap applied, no-cap-when-absent, missing-cost calculation_error, invalid-ratio error, needs_review gate, dispatch via calculate_base_policy_support, normalization into the standard result).
- Validator: added `cost_share` to `ALLOWED_CALCULATION_TYPES`, added `support_ratio`/`max_support_amount`/`cost_field` to the allowed support/item fields, and validate `support_ratio` (required number) and `max_support_amount` (number when present). Added validator tests for valid cost_share, missing support_ratio, and null max_support_amount.
- Prompt: updated `prompts/policy_extraction_v6.md` with a "Use cost_share when" section and shared-rule entry, instructing the model to use cost_share for percentage-of-cost subsidies and never fake a monthly_amount.
- Updated `docs/POLICY_SCHEMA.md` with a Cost-Share Calculation Type section.
- Live verification: extracting `flexible_work_system_support` with the updated prompt now yields two `cost_share` items (`support_ratio=0.8 max=10,000,000` and `support_ratio=1.0 max=1,800,000`) with validation passing 0 errors. Previously this document was wrongly extracted as `monthly_fixed` with an invented `monthly_amount` (e.g. 833,333). The flexible_work_system gold draft (currently 0 items / all unresolved) should be updated to use cost_share.
- Verification: 13 calculation/recommendation/validator tests pass with no regression. Full verify.ps1 not run (sandbox blocks -ExecutionPolicy Bypass).

## 2026-06-17 Monthly Wage Cap (per-wage monthly ceiling)

- Scope: added a per-month wage cap end-to-end (engine, validator, domain adapter, extraction prompt) so subsidies that cap the monthly amount at a fraction of wage (e.g. replacement_workshare "월별 지원액은 임금액의 80%를 초과할 수 없음") are computed instead of dropped to unresolved_rules. This is the next roadmap Stage 8 calculation-gap closure after OR groups and cost_share. Plan file: C:\Users\fixup\.claude\plans\a-peaceful-teacup.md.
- Decisions (confirmed with user): cap applies to monthly_fixed + period_tiered; missing wage uses an explicit configurable assumed wage (ASSUMED_DEFAULT_MONTHLY_WAGE = 3,000,000) with an `assumed_wage_used` warning step rather than silently fabricating or erroring; cap is active only when a support item declares `monthly_cap_ratio`, so policies without it are unchanged.
- Engine (`services/calculation_service.py`): added `ASSUMED_DEFAULT_MONTHLY_WAGE`, `DEFAULT_WAGE_FIELD`, and `apply_monthly_wage_cap(monthly_amount, support, rule_input, tier_index=None)` returning `(effective_monthly, cap_step or None)`. Formula `effective = min(policy_monthly, round(wage * monthly_cap_ratio))`; wage read via `get_nested_value(rule_input, cap_field)` (default `employee.monthly_wage`); reasons `wage_cap_applied` / `wage_cap_not_binding` / `assumed_wage_used`. Integrated into `calculate_monthly_fixed_policy_support` (cap before the multiply, cap step inserted before the monthly_fixed_calculation step) and `calculate_period_tiered_policy_support` (each tier capped by the same ceiling, one cap step per capped tier). No change to `normalize_policy_calculation_result`; the warning lives in `calculation_steps`.
- Tests: added `match_agent_v0.8/match_agent_v0.8/test_monthly_wage_cap.py` (cap binding, cap not binding, assumed-wage warning, period_tiered per-tier cap, no-ratio no-op, helper no-op).
- Validator (`services/policy_extraction_candidate_validator.py`): added `monthly_cap_ratio`/`cap_field` to the allowed item/support fields and validate `monthly_cap_ratio` is a number in (0,1] (new error `invalid_monthly_cap_ratio`). Added validator tests.
- Domain adapter (`services/rule_engine_domain_adapter.py`): pass through `employee.monthly_wage` (forwards a provided value only; None when absent). Updated the adapter's full-rule_input equality test for the new field.
- Prompt/docs: updated `prompts/policy_extraction_v6.md` to emit `monthly_cap_ratio` for per-wage caps (no longer unresolved_rules), and added Monthly Wage Cap sections to `docs/POLICY_SCHEMA.md` and `docs/RULE_ENGINE.md`.
- Live verification: extracting `replacement_workshare_support` now emits `monthly_cap_ratio: 0.8` on all three support items (1.4M/1.3M/1.2M), and the per-wage cap is no longer in unresolved_rules. One unrelated `evidence_not_in_raw_text` remains on a different unresolved rule (the known multi-cell table-merge issue), not caused by this feature.
- Verification: 15 calculation/recommendation/validator/adapter tests pass with no regression (incl. demo recommendation API). Full verify.ps1 not run (sandbox blocks -ExecutionPolicy Bypass).
- Follow-ups (out of scope here): conditional_bonus's own bonus monthly cap; monthly_exclusion_rule (월 부지급); a dedicated top-level `warnings` field; moving ASSUMED_DEFAULT_MONTHLY_WAGE to env/config.

## 2026-06-17 Monthly Exclusion (해당월 부지급)

- Scope: added per-month exclusion end-to-end (engine, validator, domain adapter, extraction prompt) so "해당월 부지급" rules (예: 출퇴근기록 누락 월 3일 초과 시 해당월 미지급) reduce paid months instead of being dropped to unresolved_rules. This is the second half of option A (after the monthly wage cap).
- Design decisions: (1) scope monthly_fixed only — period_tiered exclusion is ambiguous (which tier loses a month), explicitly out of scope. (2) The exact trigger (attendance violation) cannot be evaluated from data the engine holds, so the support item only carries a boolean marker `monthly_exclusion: true`; the count of non-payable months is a provided input (`excluded_months_field`, default `leave_event.excluded_months`). (3) Missing count means 0 excluded (full payment) with step reason `no_exclusion_data` — nothing is inferred. Active only when the marker is present, so existing policies are unchanged.
- Engine (`services/calculation_service.py`): added `DEFAULT_EXCLUDED_MONTHS_FIELD` and `apply_monthly_exclusion(eligible_months, support, rule_input)` returning `(adjusted_eligible_months, exclusion_step or None)`; `adjusted = max(0, eligible_months - excluded_months)`. Integrated into `calculate_monthly_fixed_policy_support` (applied to eligible_months before the multiply, exclusion step inserted before the wage-cap step; result `eligible_months` reflects the adjusted value). Composes with the wage cap (cap affects amount, exclusion affects months).
- Tests: added `match_agent_v0.8/match_agent_v0.8/test_monthly_exclusion.py` (excluded reduces months, missing data pays full with note, excluded exceeds eligible floors at 0, no-marker no-op, combined with wage cap, helper no-op).
- Validator: added `monthly_exclusion`/`excluded_months_field` to allowed item/support fields and validate `monthly_exclusion` is boolean (new error `invalid_monthly_exclusion`). Added validator tests.
- Domain adapter: pass through `leave_event.excluded_months`; updated the adapter's full-rule_input equality test.
- Prompt/docs: `prompts/policy_extraction_v6.md` now instructs setting `monthly_exclusion: true` on the affected support item for per-month nonpayment rules (removed `monthly_exclusion_rule` from the unresolved_rules list); added Monthly Exclusion sections to `docs/POLICY_SCHEMA.md` and `docs/RULE_ENGINE.md`.
- Live verification: extracting `childcare_flexible_start_support` now emits `monthly_exclusion: true` on the support item, keeps the OR child-age condition group, drops `monthly_exclusion_rule` from unresolved_rules (only complex_headcount_cap / shared_duration_cap / ambiguous_target_policy remain — the next features), and validation passes with 0 errors.
- Verification: 16 calculation/recommendation/validator/adapter/demo-API tests pass with no regression. Full verify.ps1 not run (sandbox blocks -ExecutionPolicy Bypass).
- Option A (monthly cap + monthly exclusion) is now complete. Remaining unresolved schema gaps for future work: complex_headcount_cap and shared_duration_cap (option B — combination/company/cross-policy level).
